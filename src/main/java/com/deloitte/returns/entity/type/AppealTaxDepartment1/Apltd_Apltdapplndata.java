package com.deloitte.returns.entity.type.AppealTaxDepartment1;
import java.util.List;
import com.fasterxml.jackson.annotation.JsonProperty;
import jakarta.persistence.CascadeType;
import jakarta.persistence.Entity;
import jakarta.persistence.GeneratedValue;
import jakarta.persistence.GenerationType;
import jakarta.persistence.Id;
import jakarta.persistence.JoinColumn;
import jakarta.persistence.OneToMany;
import jakarta.persistence.OneToOne;
import jakarta.persistence.Table;
import lombok.AllArgsConstructor;
import lombok.Data;
import lombok.NoArgsConstructor;

@Data
@AllArgsConstructor
@NoArgsConstructor
@Entity
@Table(name = "apltdapplndata", schema = "appeal_by_tax_department")

public class Apltd_Apltdapplndata {
	@Id
	@GeneratedValue(strategy = GenerationType.IDENTITY)
	private Long id;

	@JsonProperty("legnm")
	private String legnm;

	@JsonProperty("trdnm")
	private String trdnm;

	@JsonProperty("filingdt")
	private String filingdt;
	
	@JsonProperty("orddtl")
	@OneToOne(cascade = CascadeType.ALL)
	@JoinColumn(name = "orddtl_id")
	private Apltd_Orddtl orddtl;
	
	@JsonProperty("issuecat")
	@OneToMany(cascade = CascadeType.ALL)
	@JoinColumn(name = "apltdapplndata_id")
	private List<Apltd_Issuecat> issuecat;

	@JsonProperty("suppdocs")
	@OneToMany(cascade = CascadeType.ALL)
	@JoinColumn(name = "apltdapplndata_id")
	private List<Apltd_Suppdocs> suppdocs;

	@JsonProperty("maindocs")
	@OneToMany(cascade = CascadeType.ALL)
	@JoinColumn(name = "apltdapplndata_id")
	private List<Apltd_Maindocs> maindocs;

	@JsonProperty("annxdocs")
	@OneToMany(cascade = CascadeType.ALL)
	@JoinColumn(name = "apltdapplndata_id")
	private List<Apltd_Annxdocs> annxdocs;

	@JsonProperty("todtls")
	@OneToOne(cascade = CascadeType.ALL)
	@JoinColumn(name = "todtls_id")
	private Apltd_Todtls todtls;


}
